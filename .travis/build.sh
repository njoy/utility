...
if [ "$TRAVIS_OS_NAME" = "linux" ]; then
  if [ "$CXX" = "clang++" ]; then
    ...
    export CUSTOM='-DCMAKE_AR=/usr/bin/gcc-ar-6 -DCMAKE_RANLIB=/usr/bin/gcc-ranlib-6 -DCMAKE_CXX_ARCHIVE_CREATE="<CMAKE_AR> qcs <TARGET> <LINK_FLAGS> <OBJECTS>" -DCMAKE_C_ARCHIVE_FINISH=TRUE'
